...
export class NewRFEDocumentComponent implements OnInit, OnDestroy {
    ...
    */
    // estRegexCheck(i): void {
    //     let estRegex = /^[0-9]*(\.?[0-9]{0,1})?$/;
    //     let currentValue = this.newRFEDocument.get('workerInfo').get([i, 'estHours']).value;
    //     if (currentValue) {
    //         if (estRegex.test(currentValue)) {
    //             this.estKeyPress = true;
    //         } else {
     ...
     */
    // rateRegexCheck(i): void {
    //     let rateRegex = /^[0-9]*(\.?[0-9]{0,2})?$/;
    //     // let currentValue = event.target.value;
    //     let currentValue = this.newRFEDocument.get('workerInfo').get([i]).value.rate;
    //     if (currentValue) {
    //         let FullCurrentValueAsString = currentValue.trim().replace(/,/gi, '');
    //         let valueBeforeDot = '';
    //         let valueAfterDot = '';
    //         let valueAfterDotLength = 0;
    //         let sliceDecimalPoint = '';
    //         if (FullCurrentValueAsString.split('.').length > 0) {
    //             valueBeforeDot = FullCurrentValueAsString.split('.')[0];
    //             if (FullCurrentValueAsString.split('.')[1] || FullCurrentValueAsString.split('.')[1] === '') {
    //                 valueAfterDot = '.' + FullCurrentValueAsString.split('.')[1];
    //                 valueAfterDotLength = FullCurrentValueAsString.split('.')[1].length;
    //                 if (valueAfterDotLength > 2) {
    //                     let latestNumberBeforeDot = +valueBeforeDot;
    //                     sliceDecimalPoint = latestNumberBeforeDot.toLocaleString('en-US') + (valueAfterDot).slice(0, 3);
    //                 }
    //             }
    //         }
    //         let latestNumberBeforeDot = +valueBeforeDot;
    //         if (rateRegex.test(FullCurrentValueAsString) && valueAfterDotLength < 2) {
    //             this.rateKeyPress = true;
    //             if (latestNumberBeforeDot) {
    //                 this.newRFEDocument.get('workerInfo').get([i]).patchValue({
    //                     rate: latestNumberBeforeDot.toLocaleString('en-US') + valueAfterDot,
    //                 });
    //             }
    //         } else {
    //             if (isNaN(+FullCurrentValueAsString)) {
    //                 this.newRFEDocument.get('workerInfo').get([i]).patchValue({
    //                     rate: null,
    //                 });
    //             } else if (valueAfterDotLength > 2) {
    //                 this.newRFEDocument.get('workerInfo').get([i]).patchValue({
    //                     rate: sliceDecimalPoint,
    //                 });
    //             }
    //             this.rateKeyPress = false;
    //         }
    //     } else {
    //         this.rateKeyPress = true;
    //     }
    ...
    */
    // regexCheckAdditionalExpenses(): void {
    // let expensesRegex = /^[0-9]*(\.?[0-9]{0,2})?$/;
    ...
    //     }
    //     let latestNumberBeforeDot = +valueBeforeDot;
    //     if (expensesRegex.test(FullCurrentValueAsString) && valueAfterDotLength < 2) {
    //         this.addexpKeyPress = true;
    //         if (latestNumberBeforeDot) {
    //             this.newRFEDocument.patchValue({
    //                 additionalExpenses: latestNumberBeforeDot.toLocaleString('en-US') + valueAfterDot,
    //             });
    //         }
    //     } else {
    //         if (isNaN(+FullCurrentValueAsString)) {
    ...
    //         } else if (valueAfterDotLength > 2) {
    //             this.newRFEDocument.patchValue({
    //                 additionalExpenses: sliceDecimalPoint,
    //             });
    //         }
    //         this.addexpKeyPress = false;
    //     }
    // } else {
    //     this.addexpKeyPress = true;
    // }
    ...
    * 
    * @param i 
    * Fixed bid amount two decimal restriction
    */
    // regexFixedBidAmount(): void {
    //     let fixedRegex = /^[0-9]*(\.?[0-9]{0,1})?$/;
    //     let currentValue = this.newRFEDocument.get('fixedBidAmount').value;
    //     if (currentValue) {
    //         if (fixedRegex.test(currentValue)) {
    //             this.fixedKeyPress = true;
    //         } else {
    //             if (isNaN(currentValue)) {
    //                 this.newRFEDocument.patchValue({
    //                     fixedBidAmount: null,
  ...
  */
    closeApprovalPopup(): void {
        const approvalControl = this.newRFEDocument.get('approval');
        if (approvalControl.value.length < 2) {
 ...
 */
    ...
    */
    ...
    checkFormValidation(form): boolean {
        ...
        } else if (!distributionCheck || this.approvalAmount === 0 || this.approverLimitValid || this.validateAccountingTable.accountInValid || this.validateAccountingTable.costCenterInValid || !approverCheckForSelfTag) {
    ...
    */
    ...
    edit(isEditApprovedForm: boolean) {
        if (isEditApprovedForm) {
            ...
            // console.log(this.newRFEDocument.get(['informationalCopy',0]))
