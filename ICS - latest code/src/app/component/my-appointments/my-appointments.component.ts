...
export class MyAppointmentsComponent implements OnInit {
  ...
  cancel(appointmentId: number, appointmentIndex: number, programId: number, recursive: boolean) {
    let cancelappointmentList: any[] = [];
    if (recursive) {
      this.appointmentList.forEach(element => {
        if (element.programId == programId) {
          cancelappointmentList.push(element.id);
        }
      });
    }

    this.dialogService.appointmentCancellation(appointmentId, recursive, cancelappointmentList).subscribe((results: any) => {
      if (results.status === 'success') {
        if (recursive) {
          this.appointmentList.splice(appointmentIndex, 1);
          for (let m = 0; m < this.appointmentList.length; m++) {
            if (this.appointmentList[m].programId == programId) {
              this.appointmentList.splice(m, 1);
              m = -1;
            }
          }
        } else {
  ...
  reschedule(programId, timeSlotId, programDateId, appointmentId, appointmentIndex) {
    this.dialogService.selectProgramDate(programId, timeSlotId, programDateId, this.isRecursive).subscribe((results: any) => {
      if (results) {
