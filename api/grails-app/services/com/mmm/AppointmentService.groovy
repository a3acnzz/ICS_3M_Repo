...
class AppointmentService {
	...
	protected managePendingAppointment(def json) {
		...
		if (json.existingTimeSlotId) {
			println json.existingTimeSlotId
			...
			println existingPendingAppointment.id
	...
	protected save(def json) {
		...
		for (appt in json) {
			...
			PendingAppointment pendingAppointment = PendingAppointment.findByTimeSlotAndCreatedBy(timeSlot, userName, [sort: "dateCreated", order: "asc"])
			pendingAppointment?.delete()
		...
		for (appt in json) {
			...
			if (programAppointment && programAppointment.size() > 0) {
				...
				if( ((appointment.maxAppointmentsPerUser - programAppointment.size()) <= 0) ) {
					//					|| ((appointment.minAppointmentsPerUser - programAppointment.size()) >= 0)
